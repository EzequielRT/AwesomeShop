    ...
    public class AddOrderCommand : IRequest<Guid>
    {
        ...
        public Order ToEntity()
        {
            return new Order(
                ...
                new PaymentAddress(PaymentAddress.State, PaymentAddress.Number, PaymentAddress.City, PaymentAddress.State, PaymentAddress.ZipCode),
                new PaymentInfo(PaymentInfo.CardNumber, PaymentInfo.FullName, PaymentInfo.Expiration, PaymentInfo.Cvv),
                Items.Select(i => new OrderItem(i.ProdutoId, i.Quantity, i.Price)).ToList()
            );
        }
    }
